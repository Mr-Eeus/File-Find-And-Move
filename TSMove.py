...
def TSRemove(filesToMove):
    ...
    for file in dirs:  # Start Initial Loop
        ...
        if "CDAC" in fileName:  # If CDAC found in the filename,
            tsFileList = [x for x in tsDirAC if
                          re.search(fileName, x)]  # For item in path, if a match for a regex search
        ...
        for i in tsFileList:  # Start Secondary Loop, Through Tablet Scans Folder
            if len(fileName) <= 7:  # If length of filename is < 7,
                try:
                    newFile = re.search(r'(CD..[0-9][0-9][0-9])\D+\w.*',
                                        i)  # Set newFile = 3-Digit Regex Search, No Digit after 3rd
                    newFile = newFile.group(0)  # Convert to string value of regex search
                    try:
                        shutil.move(tsPath + newFile,
                                    tsMoveToLocation + newFile)  # Move to Tablet Scan Path determined by fileName
                ...
                try:
                    shutil.move(tsPath + i,
                                tsMoveToLocation + i)  # For Each Item, Find in TS Location, Move to Discontinued Folder
        ...
        if "CDAC" in fileName:
            mlFileList = [x for x in mlDirAC if re.search(fileName,
                                                          x)]  # Initialize list matching skus pulled by Regex statement, Master List
            mLPath = mlPathAC
        ...
        for i in mlFileList:  # Start Tertiary Loop
            if len(fileName) <= 7:
                ...
                try:
                    shutil.move(mLPath + i,
                                moveToLocation + i)  # For Each Item, Find in ML Location, Move to Discontinued Folder
        ...
        if "CDAC" in fileName:
            iTFileList = [x for x in itDirAC if re.search(fileName,
                                                          x)]  # Initialize list matching skus pulled by Regex statement, Item Tagged List
            iTPath = itPathAC
        elif "CDHO" in fileName:
        ...
        for i in iTFileList:  # Start Quaternary Loop
            if len(fileName) <= 7:
                ...
                try:
                    shutil.move(iTPath + i,
                                moveToLocation + i)  # For Each Item, Find in IT Location, Move to Discontinued Folder
                    print("Moved : " + i + " to " + moveToLocation)
                except FileNotFoundError:
                    print('[EXPECTED EXCEPTION] File Not Found In ' + iTPath + ' - Continuing..')
                    unfoundFiles.append(i)
